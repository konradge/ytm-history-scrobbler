...
class TitleAuthorBuilder:
    ...
    def __init__(self):
        self.ignore_channel = set(["tldp", "rhino", "subpop", "mariemauricei"])
        self.conflicts = {}

    def build_title_author(self, video_title: str, channel_name: str) -> tuple[str, str, bool]:
        ...
        if len(video_title.split(" - ")) > 1:
            ...
            if channel_name.replace(" ", "").lower() in self.ignore_channel:
                ...
                return title_from_video_title, author_from_video_title, True
            ...
            if channel_name.replace(" ", "").lower() in normalized_author_from_video_title.lower():
                ...
                return video_title, channel_name, False
        else:
            # ok, video title does not contain any author
            return video_title, channel_name, True
    ...
    def special_cases(self, title: str, channel: str) -> Optional[tuple[str, str]]:
        if channel == "CookieAdventures" and "The Social Network" in title:
            return f"{title.split("-")[0].strip()} (From The Social Network)", "Trent Reznor & Atticus Ross", False
        elif channel == "zombiphile68" and "Death Proof" in title:
            a, t = title.split(" - ")
            return t.replace("Death Proof", "From Death Proof").strip(), a.split(" ", 1)[1].strip(), False
        elif channel == "SonySoundtracks":
            # TODO
            return "", "", False
        elif channel == "Nicholas Britell":
            return title, channel, False
        elif channel == "Linkin Park":
            return title.split(" - ")[0].strip(), channel, False
